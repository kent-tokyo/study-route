...
## Phase 9.8: レイアウト改善・ドキュメント整理（進行中）

- [x] **STARTエリアの左端配置**: 全体マップでSTARTバッジ付きエリア（入力エッジなし）を最も左（低x座標）に配置。哲学ドメインで論理学・倫理学・美学をx=0に、認識論・形而上学をx=400に調整
- [x] **lessons.md統合**: ルートの`lessons.md`（旧plactice_math時代）と`tasks/lessons.md`を統合。認証方式・Next.js 16・Tailwind CSS v4の教訓を追加、ルートファイルを削除
- [x] **ドキュメント更新**: requirements.md（AWSエリア数5→8修正、CSノード数28→25修正、STARTエリア左端配置の記述追加）、todo.md更新
- [ ] **xx-003コンテンツ生成**: modal-logic, ecs, automata の3ノードのstandard生成（進行中）
